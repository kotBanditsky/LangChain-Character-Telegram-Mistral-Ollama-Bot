...
class Text2ImageAPI {
  ...
  async generate(
    ...
    const params = {
      ...
      height,
      style: styles[style],
      negativePromptUnclip: config.NEGATIVE_PROMT,
      generateParams: {
        query: prompt,
      },
    };
  ...
  async checkGeneration(requestId, attempts = 50, delay = 20) {
    while (attempts > 0) {
      try {
...
async function generateImage(response) {
  ...
  const api = new Text2ImageAPI(
    ...
    config.KANDINSKY_KEY,
    config.KANDINSKY_SECRET
  );

  const modelId = await api.getModels();
  const uuid = await api.generate(response, modelId, 1, 500, 500, 1);
